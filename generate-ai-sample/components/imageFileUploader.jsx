...
import { removeBackground, generateBackground } from "@/api/photoRoom";
...
const ImageFileUpload = (props) => {
  ...
  const generateImage = async (e, setter) => {
    ...
    const res = await generateBackground(imageFile, prompt.value);
    setter(res);
  };
    ...
    <ImageFileUploadWrapper>
      ...
      <div className="common_input_block">
        ...
        <div className="image_support_msg">
          ...
          <Button
            variant="contained"
            onClick={(e) => generateImage(e, setGenImageFile)}
          >
            背景を生成
          </Button>
        </div>
        <div style={{ marginTop: "16px" }}>
          <label className="image_upload_wrapper">
            {genImageFile ? (
              <img
                src={
                  typeof genImageFile === String.name.toLocaleLowerCase()
                    ? genImageFile
                    : URL.createObjectURL(genImageFile)
                }
                alt="Gen Image file"
                width="380"
                height="300"
